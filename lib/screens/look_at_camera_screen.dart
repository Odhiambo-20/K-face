...
class _LookAtCameraScreenState extends State<LookAtCameraScreen>
  ...
  static const int _tickRingCount = 72;
  ...
  */
  ...
  double _recordingRingProgress = 0.0;
  int _countdownValue = _countdownSeconds;
  Timer? _countdownTimer;
  Timer? _recordTimer;
  Timer? _ringProgressTimer;
  ...
  Future<void> _safeSetExposureLocked(CameraController controller) async {
    try {
      ...
      final targetOffset = 0.5;

      await controller.setExposureOffset(
        targetOffset.clamp(0.0, maxExposure),
      );
  ...
  */
  ...
  void _startRecordingRingProgress() {
    ...
    final stepDuration = Duration(
      milliseconds: ((_recordDurationSeconds * 1000) / _tickRingCount).round(),
    );
    var activeTicks = 0;

    _ringProgressTimer = Timer.periodic(stepDuration, (timer) {
      ...
      activeTicks++;
      final nextProgress = (activeTicks / _tickRingCount).clamp(0.0, 1.0);
      setState(() {
        _recordingRingProgress = nextProgress;
      });

      if (activeTicks >= _tickRingCount) {
        timer.cancel();
      }
    });
  }
  ...
  Future<void> _uploadVideo(XFile videoFile, {String? frameUrl}) async {
    ...
    */
    ...
    try {
      result = widget.draft.isLogin
          ? await _api.uploadLogin(
              draft: widget.draft,
              videoFile: localVideoFile,
              frameWidth: _lastFrameW,
              frameHeight: _lastFrameH,
            )
          : await _api.uploadEnrollment(
              draft: widget.draft,
              videoFile: localVideoFile,
              frameWidth: _lastFrameW,
              frameHeight: _lastFrameH,
      ...
      setState(() {
        _uploadErrorDetail =
            '${widget.draft.isLogin ? 'Login' : 'Enrollment'} failed.\nPOST $url\n\n$e';
        _uploadProgress = 0.0;
      });
      _setStatus(widget.draft.isLogin
          ? 'Could not complete login. Try again.'
          : 'Could not complete enrollment. Try again.');
      await _restartTrackingIfNeeded();
      return;
    }

    // Parse structured helper data from backend response
    Map<String, dynamic> extraMeta = {};
    if (result.rawBody != null) {
      try {
        extraMeta = jsonDecode(result.rawBody!) as Map<String, dynamic>;
      } catch (_) {}
    }

    // ── 2. Firebase save — isolated, never allowed to throw upward ────────────
    if (_firebase.isReady) {
      try {
        if (widget.draft.isLogin) {
          await _firebase.saveLoginRecord(
            username: widget.draft.username,
            success: result.ok,
          ...
          await _firebase.saveSignUpRecord(
            username: widget.draft.username,
            success: result.ok,
            hashkey: result.hashkey,
            message: result.message,
        ...
        debugPrint('[Firebase] Save failed (non-fatal): $firebaseError');
      }
    }
    ...
    if (result.ok) {
      if (result.hashkey.isEmpty) {
        setState(() {
          _uploadErrorDetail =
              'Succeeded but no key was returned.\nRaw: ${result!.rawBody}';
          _uploadProgress = 0.0;
        });
        _setStatus('Error: missing key.');
        _didTriggerCapture = false;
        await _restartTrackingIfNeeded();
        return;
      }
  ...
  // Saves a failure record to Firebase without risking a throw on the happy path.
  void _saveFailureToFirebase({
    required String? firebaseVideoUrl,
    required String url,
    ...
    final isLogin = widget.draft.isLogin;
    final username = widget.draft.username;
    final frameW = _lastFrameW;
    final frameH = _lastFrameH;
    Future.microtask(() async {
      try {
        ...
        debugPrint('[Firebase] Failure record save failed: $e');
      }
    });
  }
